...
def hist(df, label, correct, i):
  df = df.loc[(df['gold-lbl'] == label) & (df['correct'] == correct)]
  #print df
  small_df = df.sort_values(by=['count'], ascending=False)[:20]
  print small_df
...
def main():
  ...
  hist(df, "entailment", "correct", 1)
  hist(df, "entailment", "wrong", 2)
  hist(df, "neutral", "correct", 3)
  hist(df, "neutral", "wrong", 4)
  hist(df, "contradiction", "correct", 5)
  hist(df, "contradiction", "wrong", 6)
