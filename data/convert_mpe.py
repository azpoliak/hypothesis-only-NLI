...
for f in ["train", "dev", "test"]:
  ...
  for line in open("mpe/mpe_%s.txt" % (f)):
    ...
    line = line.split("\t")
    if len(line) != 10:
        print("bad line: ", len(line), " ",  line)
        warnings.warn("MPE %s file has a bad line at line numbers %d" % (f, line_count))
  ...
  if len(hypoths) == len(set(hypoths)):
      warnings.warn("A hypothesis appears more than once")
